...
class AiSettingController extends Controller
{
    public function index(): View
    {
        ...
        return view('admin.ai.index', compact('setting', 'usage', 'recentUsage'));
    }
    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        if ($setting) {
            ...
            if (blank($validated['api_key'] ?? null)) {
                return back()->withErrors(['api_key' => 'An API key is required for the first AI configuration.'])->withInput();
            }
    ...
    public function test(Request $request, AiService $ai): RedirectResponse
    {
        try {
            ...
            return back()->withErrors([
                'ai_test' => 'The AI connection test failed. Check the provider, model, endpoint, API key and account availability. Provider details are hidden for security.',
            ]);
        }
    }
}
