...
<div class="course-meta"><span class="badge">{{ ucfirst((string) data_get($enrolment,'status','enrolled')) }}</span>@if(data_get($enrolment,'completed_at'))<span><i class="fas fa-circle-check"></i> Completed</span>@endif</div>
@if($enrolment->course)<a class="btn btn-primary" href="{{ route('public.courses.show',$enrolment->course) }}">Continue learning</a>@endif
</article>
@empty
