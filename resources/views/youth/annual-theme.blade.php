...
@if(!empty($theme->image_path))<img src="{{ asset('storage/'.$theme->image_path) }}" alt="{{ $theme->theme }}">@endif
<div class="theme-body"><span class="theme-year">{{ $theme->year }}</span><h2>{{ $theme->theme }}</h2>@if(!empty($theme->scripture_reference))<p class="scripture"><i class="fas fa-book-bible"></i> {{ $theme->scripture_reference }}</p>@endif @if(!empty($theme->description))<p class="muted">{{ $theme->description }}</p>@endif</div>
</article>
@empty
