...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box} body{font-family:Arial,Helvetica,sans-serif;margin:0;background:var(--bg);color:var(--text)} a{text-decoration:none;color:inherit} button,input,select,textarea{font:inherit}
        ...
        @media(max-width:800px){.wrap{grid-template-columns:1fr}.side{display:none}.side.open{display:block}.mobile-menu-btn{display:inline-flex}.main{padding:16px}.grid,.campaign-grid{grid-template-columns:1fr}.page-head{align-items:flex-start;flex-direction:column}.filters,.form-grid,.detail-grid{grid-template-columns:1fr}.span-2{grid-column:span 1}.top{padding:0 14px}.top .user-name{display:none}.modal{padding:10px}.modal-card{max-height:94vh;padding:16px}.toolbar{align-items:stretch}.toolbar .filters{width:100%}}
    </style>
    @stack('styles')
</head>
<body>
<div class="top">
    <div style="display:flex;align-items:center;gap:10px"><button class="icon-btn mobile-menu-btn" id="menuToggle" type="button"><i class="fas fa-bars"></i></button>
    @php try { $sysName = \App\Models\SiteSetting::get('system_name','COU Youth Platform CMS'); $sysLogo = \App\Models\SiteSetting::get('logo'); } catch (\Throwable $e) { $sysName='COU Youth Platform CMS'; $sysLogo=null; } @endphp
    <a class="brand" href="{{ route('admin.dashboard') }}">@if(!empty($sysLogo))<img src="{{ asset('storage/'.$sysLogo) }}" height="28" alt="logo">@else<i class="fas fa-church"></i>@endif<span>{{ $sysName }}</span></a></div>
